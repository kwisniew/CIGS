...
module Impedance_tests_2D_CIGS_grid
...
function main(;nref=0,Plotter=nothing,verbose=false, unknown_storage=:sparse,
    ...
    ## Experimental grid from CIGS example
    width_ZnO  = 0.5 * μm 
    ...
    x0              = 0.0 * μm
    δ               = 3*3        # the larger, the finer the mesh
    ...
    Plotter.figure(3)
    Plotter.surf(X[:], Y[:], steadystate[1, :], label="solution")
    Plotter.title("Solution")
