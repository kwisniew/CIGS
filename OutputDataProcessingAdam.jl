...
function display() #function to display plots
    ...
    plot1=plot(bias,y, reuse=false, xlabel = "Voltage[U]", ylabel="Capacitance")

    model(x) = coef(ols)[1] + coef(ols)[2] * x
    #1.4775483873006742e7
    build_in_potential = coef(ols)[1]/coef(ols)[2]
    scatter(data.X, data.Y)
    plot2= plot(data.X, model.(data.X), legend=false, reuse = false, xlabel = "Voltage [U]", ylabel="1/C^2")
