...
module Example_CIGS_2_Schottky_1D_transient_RDLTS
...
function main(;n = 3, voltageStep=0.5, Plotter = PyPlot, plotting = false, verbose = false, test = false, unknown_storage=:sparse)
    ...
    solution              = FileIO.load("solution.jld2","solution")
    ...
    if plotting 
        ...
        ## ##### set legend for plotting routines #####
        Plotter.figure()
        plot_energies(Plotter, grid, data, solution, "Starting condition", label_energy)
        Plotter.figure()
        plot_densities(Plotter, grid, data, solution,"Starting condition", label_density)
    end
    ...
    Voltage_step         = 0#voltageStep            # final bias value
